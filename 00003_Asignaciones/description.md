1. Traduzca a lenguaje C los siguientes programas escritos en pseudocódigo:


<img src="https://raw.githubusercontent.com/algo1-unc/mumuki-guia-c-laboratorio-1-imperativa-algo-1-unc/master/assets/2020-10-06-002613_754x147_scrot_1602341264532.png" alt="2020-10-06-002613_754x147_scrot_1602341264532.png" width=100% >

> **Importante**: "Para declarar un determinado estado inicial (x->, y->), asigne valores a `X_INIT` e `Y_INIT`".

2.  Complete la tabla de abajo para diferentes estados iniciales. Para ello, asigne `DEBUG true` y envie su solución para obtener los valores de los estados finales de cada una de las funciones programadas.

<img src="https://raw.githubusercontent.com/algo1-unc/mumuki-guia-c-laboratorio-1-imperativa-algo-1-unc/master/assets/2020-10-10-114906_631x290_scrot_1602341360424.png" alt="2020-10-10-114906_631x290_scrot_1602341360424.png" width=100%>

2. Ahora asigne `DEBUG false` para activar el modo testeo y envie su solución para que el sistema pueda comprobar la corretitud del ejercicio.


> **NOTA:** No modifique el nombre de las funciones de la plantilla presentada. Complete solo donde diga `-> CODIGO AQUI <-`.
